...
public class BannerLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private void layoutLeftItem(RecyclerView.Recycler recycler) {
        ...
        if (firstChild != null) {
            ...
            layoutDecoratedWithMargins(viewForPosition, left, top, right, top + getDecoratedMeasuredHeight(viewForPosition) + mBottom);
        }
    }

    private int layoutItem(View viewForPosition, int offsetX) {
        layoutDecoratedWithMargins(viewForPosition, offsetX, getItemTop(viewForPosition), offsetX + itemWidth, getItemTop(viewForPosition) + getDecoratedMeasuredHeight(viewForPosition) + mBottom);
        return itemWidth;
    }
    ...
    private int scrollToRight(int dx, RecyclerView.Recycler recycler, int realScroll) {
        while (true) {
            ...
            if (left + Math.abs(dx) > getPaddingLeft()) {
                ...
                layoutDecoratedWithMargins(addView, left - getDecoratedMeasuredWidth(addView), getItemTop(addView), left, getItemTop(addView) + getDecoratedMeasuredHeight(addView) + mBottom);
    ...
    private int scrollToLeft(int dx, RecyclerView.Recycler recycler, int realScroll) {
        while (true) {
            ...
            if (decoratedRight - dx < mOrientationHelper.getTotalSpace()) {
                ...
                layoutDecoratedWithMargins(lastViewAdd, left, getItemTop(lastViewAdd), left + getDecoratedMeasuredWidth(lastViewAdd), getItemTop(lastViewAdd) + getDecoratedMeasuredHeight(lastViewAdd) + mBottom);
            } else {
                break;
            }
        }
